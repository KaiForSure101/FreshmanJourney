    ...
    for (int y = 0; y < static_cast<int>(layout.size()); ++y) {
        for (int x = 0; x < static_cast<int>(layout[y].size()); ++x) {
            if (player != nullptr && player->getX() == x && player->getY() == y) {
                std::cout << "\xF0\x9F\xA7\x91";
            } else {
                switch (layout[y][x]) {
                    case '#': std::cout << "\xE2\x96\x88\xE2\x96\x88"; break;
                    case 'D': std::cout << "\xF0\x9F\x8F\xA0"; break;
                    case 'L': std::cout << "\xF0\x9F\x93\x9A"; break;
                    case 'G': std::cout << "\xF0\x9F\x92\xAA"; break;
                    case 'S': std::cout << "\xF0\x9F\x8E\xAF"; break;
                    case 'C': std::cout << "\xF0\x9F\x93\x96"; break;
                    case 'F': std::cout << "\xF0\x9F\x8D\xBD\xEF\xB8\x8F"; break;
